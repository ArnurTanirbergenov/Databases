...
create view  cheap_and_light_products as 
select * from products 
where weight < 10 and price < 100;

select * from cheap_and_light_products

--We can't insert because in our table we have id as serial primary key so we should'n write it
insert into cheap_and_light_products (name, department, price, weight)
VALUES (101, 'Awesome Hot Chips', 'Grocery', 10, 2);


insert into cheap_and_light_products (name, department, price, weight)
VALUES (103, 'Soft Flanel Shirt', 'Clothes', 105, 1);


create view order_user as
select u.first_name || ' ' || u.last_name as full_name, o.id
from users u join orders o on o.user_id = u.id;

select full_name, o.order_date
from order_user ou join orders o on ou.id = o.id
where o.shipment_date is NULL;

select full_name
from order_user ou join orders o on ou.id = o.id
	join products p on o.product_id = p.id
where department = 'Tools' and paid = true;



select distinct full_name
from order_user ou join orders o on o.id = ou.id
...
--Shouldn't we check is paid = true, bc otherwise they didn't spend money
create view user_sum as
select u.id, u.first_name, u.last_name, sum(p.price) as total_spending
from users u
...
select * from users where first_name = 'Arnur';
select* from products where name = 'Transaction1';
...
BEGIN;
	update products set price = 499
		where name = 'Transaction1';
		
	select count(*) from users;
commit;

BEGIN;
	update products set price = 399
		where id = 101;
